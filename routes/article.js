...
function readArticles(userIdentity){
    let articles = JSON.parse(fs.readFileSync("./db/articles.json").toString());
    articles.sort(function(n1, n2){
        return n2.id - n1.id;//내림차순
    });
    if (userIdentity) {
        articles = articles.filter((article) => { return article.identity === userIdentity; });
    }
    return articles;
}
...
function writeArticle(title, contents, identity, req){
    ...
    const user = users.filter(function(user){//filter는 배열을 반환
...
router.get('/user/:identity/articles', function(req, res){
    if(!req.session.userInfo){
        return res.status(401).send("Unauthorized");
    }
    res.json(readArticles(req.params.identity));
});

router.put('/:articleId/certificate', function(req, res){
    if(!req.session.userInfo || !req.session.userInfo.identity || req.session.userInfo.identity !== 'admin'){
        return res.status(401).send("Unauthorized");
    }
    const certification = req.body.certification;
    changeCertification(parseInt(req.params.articleId), certification);
    
    res.send("success");
});

function changeCertification(id, certification){
    const articles = readArticles();
    const article = articles.filter(function(article){//filter는 배열을 반환
        return article.id === id;
    })[0];
    ...
    const user = users.filter((user) => {
        return user.identity === article.identity;
    })[0];
    
    if(article){
        article.certification = certification;
        user.point += certification === 'Y' ? 50 : -50;
        fs.writeFileSync('./db/articles.json', JSON.stringify(articles, null, 2));
        fs.writeFileSync('./db/users.json', JSON.stringify(users, null ,2));
    }
}
